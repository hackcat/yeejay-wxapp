  ...
  uploadCover: function () {
    ...
    wx.chooseImage({
      ...
      success: function (res) {
        ...
        // 如果有了bookId，直接上传图片
        if (that.data.bookId) {
          getApp().uploadImage({ bookId: that.data.bookId, idx: 0, imageUrl: res.tempFilePaths[0] }, function (data) {
            let resDate = JSON.parse(data);
            that.setData({
              bookCover: resDate.payload.url
            });
          });
        } else {
          getApp().uploadImage({ bookId: '', idx: 0, imageUrl: res.tempFilePaths[0] }, function (data) {
            let resDate = JSON.parse(data);
            that.setData({
